...
export default function VideoManager({ initialVideos }: { initialVideos: Video[] }) {
  ...
  function handleVideoUrlChange(url: string) {
    ...
    setEditing((prev) => ({
      ...prev!,
      videoUrl: url,
      // N'écrase la miniature que si elle est vide OU si c'est déjà une miniature YouTube
      ...
      <div className="flex flex-col gap-2">
        {videos.map((v) => (
          <div key={v.id} className="flex items-center gap-4 bg-[#161614] border border-[#1f1d1a] px-5 py-4 hover:border-[#3a3a35] transition-colors">
            ...
            <button onClick={() => { setEditing(v); setThumbError(false); }} className="font-mono text-[10px] tracking-[.16em] uppercase text-[#7a7770] hover:text-[#e9e5da] px-2">
              Modifier
            </button>
            <button onClick={() => remove(v.id)} className="font-mono text-[10px] tracking-[.16em] uppercase text-[#7a7770] hover:text-red-400 px-2">
              Suppr.
            </button>
          </div>
        ))}
        ...
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-6" onClick={(e) => e.target === e.currentTarget && setEditing(null)}>
          <div className="bg-[#141412] border border-[#2a2a26] w-full max-w-[580px] max-h-[90vh] overflow-y-auto p-8">
            ...
            <div className="flex flex-col gap-4">
              ...
              {/* URL Vidéo — traité spécialement pour l'auto-miniature */}
              <div>
              ...
              {/* Aperçu + champ miniature */}
              <div>
                <label className="block font-mono text-[10px] tracking-[.2em] uppercase text-[#7a7770] mb-1.5">
                  URL Miniature
                  {ytIdPreview && !editing.thumbnailUrl && (
                    <span className="ml-2 text-[#7a7770] normal-case tracking-normal font-sans text-[10px]">
                      (auto-rempli depuis YouTube)
                    </span>
                  )}
                </label>
                  ...
                  <div className="mb-2 relative aspect-video w-full bg-[#0f0f0e] border border-[#2a2a26] overflow-hidden">
                    {!thumbError ? (
                      <img
                        ...
                        onError={() => {
                          // Fallback hqdefault si maxresdefault absent
      ...
      <div className="flex flex-col gap-2">
        {videos.map((v) => (
          <div key={v.id} className="flex items-center gap-4 bg-[#161614] border border-[#1f1d1a] px-5 py-4 hover:border-[#3a3a35] transition-colors">
            ...
            <button onClick={() => setEditing(v)} className="font-mono text-[10px] tracking-[.16em] uppercase text-[#7a7770] hover:text-[#e9e5da] px-2">
              Modifier
            </button>
            <button onClick={() => remove(v.id)} className="font-mono text-[10px] tracking-[.16em] uppercase text-[#7a7770] hover:text-red-400 px-2">
              Suppr.
          ...
          <div className="bg-[#141412] border border-[#2a2a26] w-full max-w-[560px] max-h-[90vh] overflow-y-auto p-8">
            ...
            <div className="flex flex-col gap-4">
              {([
                ...
                ["year",         "Année",         "number"],
                ["festival",     "Festival",      "text"],
                ["thumbnailUrl", "URL miniature", "url"],
                ["videoUrl",     "URL vidéo",     "url"],
                ["order",        "Ordre",         "number"],
              ] as [keyof typeof editing, string, string][]).map(([key, label, type]) => (
                <div key={key}>
                  <label className="block font-mono text-[10px] tracking-[.2em] uppercase text-[#7a7770] mb-1.5">
